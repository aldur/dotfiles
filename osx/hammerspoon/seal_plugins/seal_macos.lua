...
function obj.ejectAll()
    ...
    if not success then
        obj.__logger.e('Got an error while ejecting all disks.')
    end
end
...
function obj.sleep()
    ...
    if not success then
        obj.__logger.e('Got an error while putting MacOS to sleep.')
    end
end

function obj.toggleWiFi()
    hs.wifi.setPower(not hs.wifi.interfaceDetails('en0').power, 'en0')
end
