...
container_file=$(dirname "$input_file_path")
container_metadata="$container_file".metadata
...
container_content="$container_file".content
...
page_index=$(jq '(.pages | index("'"$page_uid"'") // -2) + 1' "$container_content")
pdf_page_index=$(jq '(.pageIndex = (.pages | index("'"$page_uid"'")) | (if has("redirectionPageMap") then .redirectionPageMap[.pageIndex] else null end) // -2) + 1' "$container_content")

container_uuid=$(basename "$container_file")
full_output_with_page="$output"/"$page_index"."$container_uuid"."$page_uid"

pdf_file="$container_file".pdf
...
ocr_output_with_page="$output"/"ocr"/"$page_index"."$container_uuid"."$page_uid".ocr.txt
